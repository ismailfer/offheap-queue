 ...
 * @since 20221016
 */
public class OffHeapDirectByteBufferFifoQueue
{
    ...
    public OffHeapDirectByteBufferFifoQueue()
    {
     ...
     */
    public OffHeapDirectByteBufferFifoQueue(String name, int capacity, boolean cyclic)
    {
        this.name = name;
    ...
    public class OffHeapQueueAppender implements QueueAppender
    {
        ...
        private boolean currMsgConsumed = false;
        ...
        {            
            buffer.put(currMsgStart, (byte)(v ? 1 : 0));
        }
    ...
    public class OffHeapQueueTailer implements QueueTailer
    {
        ...
        public boolean nextMessage()
        {
            ...
            if (newData)
            {
                ...
                currMsgConsumed  = readMsgIsConsumed();
        ...
        public boolean nextMessage2()
        {
            ...
            if (newData)
            {
                ...
                // do a CAS to read take on this message
                while (true)
                {
                    // check if this msg is consumed
                    boolean msgConsumed = buffer.get(currMsgStart + Integer.BYTES) == 1;

                    
                    // if msg consumed; move to the next
                    if (msgConsumed)
                    {
                        msgReadCount++;
                        
                        queueMsgCount = getMsgCount();                        
                        if (msgReadCount >= queueMsgCount)
                            break;
                    }
                    // msg not consumed
                    else
        ...
        private boolean readMsgIsConsumed()
        {
